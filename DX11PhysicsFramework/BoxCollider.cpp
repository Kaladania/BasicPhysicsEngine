...
void BoxCollider::SetExtents(Vector3 extents)
{
	...
	_minPoint = Vector3(_center.x - (_extents.x / 0.5f), _center.y - (_extents.y / 0.5), _center.z - (_extents.z / 0.5f));
	_maxPoint = Vector3(_center.x + (_extents.x / 0.5f), _center.y + (_extents.y / 0.5), _center.z + (_extents.z / 0.5f));
}
...
bool BoxCollider::CollidesWith(BoxCollider* other)
{
	...
	//calculates the total (radius) size of the availble collission area (if you lay both bounding spheres next to each other)
	float combinedRadii = (this->_extents.x / 0.5f) + other->GetExtents().x;
	_debugOutputer->PrintDebugString("Combined Radii is: " + std::to_string(combinedRadii));
	...
	//calculates the total (radius) size of the availble collission area (if you lay both bounding spheres next to each other)
	combinedRadii = (this->_extents.y / 0.5f) + other->GetExtents().y;
	_debugOutputer->PrintDebugString("Combined Radii is: " + std::to_string(combinedRadii));
	...
	//calculates the total (radius) size of the availble collission area (if you lay both bounding spheres next to each other)
	combinedRadii = (this->_extents.y / 0.5f) + other->GetExtents().z;
	_debugOutputer->PrintDebugString("Combined Radii is: " + std::to_string(combinedRadii));
